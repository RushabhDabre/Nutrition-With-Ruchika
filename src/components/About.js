...
function AboutBlock({ icon, title, children }) {
  return (
    <Box sx={{ mb: 3.25 }}>
      <Typography sx={{ fontSize: '1rem', fontWeight: 700, mb: 1.25, display: 'flex', alignItems: 'center', gap: 1.25 }}>
        <Box
          sx={{
            width: 30, height: 30, borderRadius: '8px', bgcolor: 'rgba(99,102,241,0.1)', color: colors.primary,
    ...
    <List dense disablePadding>
      {items.map((item) => (
        <ListItem key={item} disableGutters sx={{ py: 0.5 }}>
          ...
          <ListItemText primaryTypographyProps={{ fontSize: '0.95rem', color: colors.textLight }} primary={item} />
        </ListItem>
      ))}
    </List>
  );
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '0.9fr 1.1fr' }, gap: 7.5, alignItems: 'center' }}>
          <Box
            sx={{
              ...
              borderRadius: '20px', aspectRatio: '3/4',
          ...
          <Box>
            <Chip label="About Me" sx={{ bgcolor: 'rgba(99,102,241,0.1)', color: colors.primary, fontWeight: 700, mb: 1.5, px: 1 }} />
            <Typography variant="h2" sx={{ fontSize: '2.3rem', mb: 2.5 }}>Hi, I'm Ruchika</Typography>
            ...
            <AboutBlock icon="📖" title="Background">
              <Typography sx={{ color: colors.textLight, lineHeight: 1.85 }}>{about.background}</Typography>
            </AboutBlock>
